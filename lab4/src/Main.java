...
public class Main {
    ...
    private static void menu(Kalendarz kalendarz) {
        ...
        while (running) {
            ...
            System.out.println("6 - dodaj 7 przykładowych spotkań w poniedziałek");
            ...
            switch (choice) {
                ...
                case 6 -> addSomeMeetings(kalendarz);
    ...
    private static void deleteMeeting(Kalendarz kalendarz) {
        int day = chooseDay();
        if (kalendarz.getDayMeetings(day).isEmpty()) {
    ...
    private static void showAllDayMeetings(Kalendarz kalendarz) {
        int day = chooseDay();
        for (Spotkanie meeting : kalendarz.getDayMeetings(day)) {
            System.out.println(meeting);
        }
    }
    private static void showAllDayMeetingsWithIndex(Kalendarz kalendarz, int day) {
        ArrayList<Spotkanie> meetings = kalendarz.getDayMeetings(day);
    ...
    private static void showDayMeetingsWithPriority(Kalendarz kalendarz) {
        int day = chooseDay();
        int priority = choosePriority();
        ArrayList<Spotkanie> meetings = kalendarz.getDayMeetingsByPriority(day, priority);
    ...
    private static void showDayMeetingsAfterTime(Kalendarz kalendarz) {
        ...
        ArrayList<Spotkanie> meetings = kalendarz.getDayMeetingsAfterTime(day, time);
        for (Spotkanie meeting : meetings) {
            System.out.println(meeting);
        }
    }
